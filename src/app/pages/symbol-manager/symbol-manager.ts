...
export class SymbolManagerComponent implements OnInit {
  ...
  ngOnInit() { this.loadStats(); this.loadIndexStats(); }

  loadIndexStats() {
    this.functionsSvc.getSymbolStats().subscribe({
      next: data => {
        const indexTickers = new Set(this.indices.map(i => i.ticker));
        const map: Record<string, SymbolStat> = {};
        for (const s of data) if (indexTickers.has(s.symbol)) map[s.symbol] = s;
        this.indexStats.set(map);
      }
    });
  }

  loadIndex(ticker: string) {
    this.loadingIndex.update(s => new Set(s).add(ticker));
    this.functionsSvc.backfillSymbol(ticker).subscribe({
      next: res => {
        this.messages.update(m => [{ symbol: ticker, text: `${res.barsAdded} bars loaded`, ok: true }, ...m]);
        this.loadingIndex.update(s => { const n = new Set(s); n.delete(ticker); return n; });
        this.loadIndexStats();
      },
      error: () => {
        this.messages.update(m => [{ symbol: ticker, text: 'Failed', ok: false }, ...m]);
        this.loadingIndex.update(s => { const n = new Set(s); n.delete(ticker); return n; });
      }
    });
  }
