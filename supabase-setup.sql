...
-- Risk results calculated daily by Azure Function
-- Monte Carlo (1000 sims), 4 horizons: 2W / 1M / 3M / 6M, 5% loss threshold
...
ALTER TABLE symbol_risk ENABLE ROW LEVEL SECURITY;
CREATE POLICY "public read symbol_risk"  ON symbol_risk FOR SELECT USING (true);
CREATE POLICY "public write symbol_risk" ON symbol_risk FOR ALL    USING (true);
